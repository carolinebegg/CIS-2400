...
void init_snake() {
    int s;
    for (s = 1; s < MAX_LENGTH; s++) {             // Initialize all (x,y) values to -1 except for snake[0]
        snake[s].x = -1;
        snake[s].y = -1;
    }
  snake[0].x = 10;
...
void reset_bombs() {
    int b;
    for (b = 0; b < MAX_BOMBS; b++) {             // Initialize all (x,y) values to -1
        bomb[b].x = -1;
        bomb[b].y = -1;
    }
  bombs_count = 0;
...
void turn_snake(int new_direction) {
    if (new_direction == DOWN && snake_direction != UP) {             // prevent a turn of more than 90 degrees
        snake_direction = DOWN;
    } else if (new_direction == UP && snake_direction != DOWN) {
        snake_direction = UP;
 ...
 */
void grow_snake() {
    snake[snake_length].x = snake[snake_length - 1].x;              // set each snake segment to the previous (x,y) 
...
int move_snake() {
    ...
    if (snake_direction == UP) {              // calculate proper move based on the direction
        head_x--;
    }
    ...
    if (head_x < 0 || head_x >= COLUMNS - 1) {              // check out of bounds moves
        return 0;
    }
    if (head_y < 0 || head_y >= ROWS) {
        return 0;
    }

    if (snake_length > 1) {
        for (i = snake_length - 1; i > 0; i--) {              // handle body movement
            snake[i].x = snake[i - 1].x;
            snake[i].y = snake[i - 1].y;
        }
    }
    snake[0].x = head_x;              // handle head movement
    snake[0].y = head_y;
 ...
 */
void spawn_fruit() {
  int x = rand16() % (COLUMNS - 2);
  int y = rand16() % (ROWS - 1);
  int i;

  for (i = 0; i < MAX_BOMBS; i++) {              // prevent spawning on a bomb
...
void spawn_bomb() {
  ...
  for (i = 0; i < bombs_count; i++) {              // prevent spawning on another bomb or fruit
    if (bomb[i].x == x && bomb[i].y == y) {
      x = rand16() % (COLUMNS - 2);
      y = rand16() % (ROWS - 1);
    }
...
int handle_collisions() {
  int fruit_collision = check_fruit_collision();
  int bomb_collision = check_bomb_collision();
  int self_collision = check_self_collision();

  if (bomb_collision != EMPTY ) {              // return collision type (if any)
    return bomb_collision;
  } else if (self_collision != EMPTY) {
...
int update_game_state() {
  int move = move_snake();
  int collision = handle_collisions();
  ...
  if (collision == FRUIT) {
    spawn_fruit();
    // lc4_puts ((lc4uint*)"\nSPAWN FRUIT!");       print statement for testing
    grow_snake();
    ...
    if (snake_length % 5 == 0) {
        spawn_bomb();
            if (timer - TIMER_DECREMENT >= MIN_TIMER) {
                timer = timer - TIMER_DECREMENT;
                // lc4_puts ((lc4uint*)"\nNEW TIMER: ");        print statement for testing
                // printnum (timer);       print statement for testing
                // endl();       print statement for testing
        }
...
void draw_snake() {
    int s;
    for (s = 0; s < snake_length; s++) {
        draw_pixel(snake[s].x, snake[s].y, GREEN);
    }
}
...
void draw_bombs() {
    int b;
    for (b = 0; b < bombs_count; b++) {
        draw_pixel(bomb[b].x, bomb[b].y, WHITE);
    }
}
 ...
 */
void display_score() {
    int x_l = index_to_pixel(1);
    int x_r = index_to_pixel(2);
    int y = index_to_pixel(1);
    int left = snake_length / 10;
    int right = snake_length % 10;
    lc4uint* l_ptr = zero;
    lc4uint* r_ptr = zero;
    if (left == 2) {
        l_ptr = two;
    ...
    if (snake_length < 10) {
      lc4_draw_sprite(x_l, y, YELLOW, r_ptr);
    } else {
      lc4_draw_sprite(x_l, y, YELLOW, l_ptr);
      lc4_draw_sprite(x_r, y, YELLOW, r_ptr);
    }
}
...
void play_game() {
    ...
    init_snake();
     // lc4_puts ((lc4uint*)"\nINIT_SNAKE");       print statement for testing
    reset_bombs();
     // lc4_puts ((lc4uint*)"\nRESET_BOMBS");       print statement for testing
    spawn_fruit();
     // lc4_puts ((lc4uint*)"\nSPAWN_FRUIT");       print statement for testing
    redraw();
     // lc4_puts ((lc4uint*)"\nREDRAW\n");       print statement for testing
    ...
    while (state == GAME_PLAYING) {
        ...
        if (key == 'i') {
            turn_snake(UP);
            // lc4_puts ((lc4uint*)"UP, ");       print statement for testing
        } 
        if (key == 'j') {
            turn_snake(LEFT);
            // lc4_puts ((lc4uint*)"LEFT, ");       print statement for testing
        } 
        if (key == 'k') {
            turn_snake(DOWN);
            // lc4_puts ((lc4uint*)"DOWN, ");       print statement for testing
        }
        if (key == 'l') {
            turn_snake(RIGHT);
            // lc4_puts ((lc4uint*)"RIGHT, ");       print statement for testing
        }
